<form wire:submit.prevent="saveComment">
    <div class="h-full">
            ...
            <div class="backMatch">
                <div class=" py-6 ">
                    <div class="bg-primary text-white m-auto text-center rounded-lg shadow-2xl w-3/4 p-2">
                        <h2 class="text-sm">{{ $match->competition->name }}</h2>
        ...
        <div id="menuTeam" class="z-10 absolute h-auto top-16 right-0 left-0 espaceCom {{ $team_action}}">
            <div class="flex flex-col items-center">
                <div class="m-4 flex flex-row justify-center">
                    <div class="flex flex-col jsutify-center">
                        ...
                        <input class="p-3 bg-white rounded shadow outline-none focus:outline-none focus:shadow-outline text-center" type="number" name="minute" wire:model="minute" placeholder="01" min="1" max="99">
                        @error('minute')
                ...
                @if($team_action == 'home')
                <select name="player" id="player" wire:model="player">
                ...
                <div class="m-4">
                    ...
                    <label for="exit" class="btn btnPrimary">Retour</label>
                </div>
                @endif
                @if($team_action == 'away')
                <div class="m-4">
                    <select class="p-3 bg-white rounded shadow outline-none focus:outline-none focus:shadow-outline w-full" name="player" id="player" wire:model="player">
                        <option value="">Choisissez un joueur</option>
                        @foreach($match->awayClub->players as $player)
                        <option class="capitalize text-sm" value="{{ $player->id}}">{{$player->first_name}} {{$player->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input class="hidden" type="radio" id="exit" wire:model="team_action" name="team_action" value="">
                    <label for="exit" class="btn btnSecondary">Retour</label>
                </div>
                @endif
            </div>
            @endif
