<div class="flex justify-between my-3 mx-1">
                ...
                @if ($compo->id === $selected_compo_id)
                    <div class="bg-primary p-2 relative">
                            ...
                            <form wire:submit.prevent="storePlayer({{ $match->homeClub->id }}, {{ $compo->id }})">
                                ...
                                <div class="flex flex-col m-1">
                                    ...
                                    <input type="text" wire:model="prenom" required
                                        class="border-l-0 border-r-0 border-t-0 text-sm focus:ring-0"
                                        placeholder="prénom">
                                </div>
                                <div class="flex flex-col m-1">
                                ...
                                <div class="flex justify-end">
                                    <button type="submit" class="btn bg-white text-primary text-xs"
                                    wire:click="openCreatePlayer">
                                    Retour
                                </button>
                        ...
                        <div class="flex flex-col">
                            ...
                            <select class="border-none text-sm focus:ring-0 m-1 text-primary"
                                wire:model="joueur_choisi">
                                <option value="">Choisir un joueur</option>
                                @foreach ($match->homeClub->players as $player)
                                    <option value="{{ $player->id }}" {{ $match->homeClub->player_of_this_match($match->id, $player->id) }}>
                                        {{ $player->full_name }} 
    ...
    <div class="flex-1 mx-1 w-1/2">
        ...
        <div class="text-sm p-2 text-right">
            @foreach ($awayCompo as $numero => $compo)
                ...
                @if ($compo->id === $selected_compo_id)
                    <div class="bg-primary p-2 relative">
                        ...
                        @if ($open_create_player)
                            <form wire:submit.prevent="storePlayer({{ $match->awayClub->id }}, {{ $compo->id }})">
                                {{-- <p class="text-xs text-secondary">Ou crée le</p> --}}
                                <div class="flex flex-col m-1">
                                    <label class="text-xs text-secondary">Prénom</label>
                                    <input type="text" wire:model="prenom" required
                        ...
                        <div class="flex flex-col">
                            ...
                            <select class="border-none text-sm focus:ring-0 m-1 text-primary"
                                ...
                                @foreach ($match->awayClub->players as $player)
                                    <option value="{{ $player->id }}" {{ $match->awayClub->player_of_this_match($match->id, $player->id) }}>
                                        {{ $player->full_name }} 
                                        {{ $match->awayClub->player_of_this_match($match->id, $player->id) == "disabled" ? "(Déjà présent)":"" }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <div class="flex justify-end">
                                <button type="button" class="btn btnSecondary text-xs"
                                    wire:click="storePlayer({{ $match->awayClub->id }}, {{ $compo->id }})">
                                    Je valide
                                </button>
                            </div>
