    ...
    <div class="mb-2">
        <div class="hidden" style="display:{{ $buttonCity == 1 ? 'block' : '' }}">
            ...
            <div>
                ...
                <input class="inputForm mb-2 w-full" placeholder="Adresse" type="text" name="inputAddress"
                    id="inputAddress" wire:model="inputAddress">
                @error('inputAddress')
        ...
        @can('update-club', $club)
            <div>
                @if ($buttonCity == 1)
                    <p class="absolute top-2 right-2 bg-danger font-bold text-xs px-2 py-1 rounded-md cursor-pointer"
                        wire:click="clickButtonCity"><i class="fas fa-times-circle"></i></p>
                @else
                    <p class="absolute top-2 right-2 bg-darkSuccess font-bold text-xs px-2 py-1 rounded-md cursor-pointer"
