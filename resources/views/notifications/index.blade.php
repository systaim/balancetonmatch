                ...
                @endif
                    ...
                    @if ($notification->type == "App\Notifications\matchEnd")
                        <a href="{{ route('matches.show', $notification->data["match_id"]) }}">
                            ...
                            <div class="flex justify-start items-center">
                                <div class="{{ $notification->read_at < now()  && $notification->read_at > now()->subWeek(1) && $notification->read_at > now()->subWeek(1)?"bg-gray-600":"bg-red-600"}} inline-block px-2 py-1 rounded-md text-white mr-6 font-bold">LIVE</div>
                                <div class="">
                                    <p class="text-sm truncate">{{ $notification->data["homeClub"] }}</p>
                                    <img src="{{ asset('images/vs-primary.png') }}" alt="versus" class="h-6 ml-3">
                                    <p class="text-sm truncate">{{ $notification->data["awayClub"] }}</p>
                                    <p class="{{ $notification->read_at < now()  && $notification->read_at > now()->subWeek(1) && $notification->read_at > now()->subWeek(1)?"bg-gray-600":"bg-primary"}} inline-block text-white rounded-lg font-bold p-2 mt-2 flex-none">Le match est terminé</p>
                                </div>
                            </div>
                        </a>
                    @endif
